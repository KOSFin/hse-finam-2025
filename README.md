...
Бекенд: https://api-hak25.ruka.me

Swagger: https://api-hak25.ruka.me/swagger

Фронтенд: https://hak25.ruka.me
...
- 🌐 **HTTP API** — эндпоинты `/radar` и `/news` позволяют получать и добавлять события через JSON.

## Как это работает

1. **Парсер** подаёт новости с полями `headline`, `url`, `entities`, `tickers`.
2. **Кластеризатор** объединяет похожие публикации и строит граф цитирования (кто у кого повторил).
3. **Скоринг** вычисляет "горячесть" по скорости распространения, достоверности и независимости источников.
4. **LLM-анализатор (опционально)** генерирует резюме, таймлайн и двуязычное объяснение *«почему сейчас»*.
...
### Добавление собственной заметки

Можно подселять свежие публикации прямо в память сервиса через POST `http://localhost:8080/news`. Минимальные поля — `headline` и `url`; остальные опциональны.

```powershell
$payload = @{
  headline = "FinTech стартап привлёк $50M"
  url = "https://example.com/news/fintech-50m"
  language = "ru"
  tickers = @("FINX")
  entities = @("Example Capital")
} | ConvertTo-Json

Invoke-RestMethod -Method Post -Uri "http://localhost:8080/news" -ContentType "application/json" -Body $payload
```

В ответ вернётся `202 Accepted` с присвоенным `id` и фактическим `published_at`. После этого событие будет учтено при следующем запросе `/radar` в рамках заданного временного окна.
